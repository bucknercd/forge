...
Strengthen safeguards and review controls for LLM-generated plans.

- make planner provenance more visible in preview output, saved reviewed plans, apply results, and JSON output
- surface clear warnings when a plan was generated by a non-deterministic (LLM-backed) planner
- optionally require reviewed-plan workflow when using LLM planners (no direct execution)
- improve diagnostics for LLM-generated plans that are schema-valid but low quality or suspicious
- ensure planner metadata is consistently recorded across preview, saved plans, and apply results
- preserve deterministic planner as the safest default path

Goal:
Make LLM-assisted planning transparent, reviewable, and safe without weakening deterministic execution guarantees.